...
<head>
    ...
    <style>
        ...
        .glass-card { background: rgba(255, 255, 255, 1); border-radius: 1.25rem; transition: all 0.4s ease; border: 1px solid #f3f4f6; }
        ...
        .search-glass { background: rgba(249,250,251,0.8); border: 1px solid #e5e7eb; transition: all 0.3s; }
        .search-glass:focus-within { background: #fff; border-color: #1c5e82; box-shadow: 0 0 0 4px rgba(28,94,130,0.1); }
    </style>
</head>
<body class="bg-gray-50 text-gray-800 font-sans flex flex-col min-h-screen selection:bg-unmerBlue selection:text-white">
    ...
    <section class="relative h-[450px] lg:h-[500px] flex items-center justify-center text-center overflow-hidden">
        ...
        <div class="relative z-10 px-6 max-w-4xl mx-auto flex flex-col items-center">
            <span class="px-4 py-1 rounded-full border border-white/20 bg-white/10 text-white/90 text-[10px] font-bold tracking-[0.3em] uppercase mb-5">Pusat Bantuan</span>
            <h2 class="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-6 tracking-tight">Knowledge Base</h2>
            <p class="text-blue-50 text-xl font-light tracking-wide max-w-2xl">Sumber daya informasi teknologi bagi sivitas akademika Universitas Merdeka Malang.</p>
        </div>
    </section>
        ...
        <div class="container mx-auto px-6 max-w-7xl relative z-10">
            <div class="text-center mb-16">
                 ...
                 <div class="max-w-md mx-auto relative search-glass rounded-2xl px-5 py-3.5 flex items-center shadow-sm">
                    ...
                    <input type="text" id="guideSearch" placeholder="Ketik topik sandi, panduan e-learning..." class="bg-transparent text-gray-700 outline-none w-full font-medium">
                 </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8" id="cards-container">
                ...
                <div class="instruction-card glass-card p-8 flex flex-col h-full group relative overflow-hidden">
                    ...
                    <div class="relative z-10 mb-6 flex-grow">
                        <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-gray-100 to-white shadow-sm flex items-center justify-center text-3xl mb-6 border border-gray-100">
                            {{ $kb->icon ?? '📄' }}
                        </div>
                        <h3 class="font-bold text-gray-900 text-lg mb-3 tracking-tight leading-snug group-hover:text-unmerBlue transition-colors">{{ $kb->title }}</h3>
                        <p class="text-gray-500 font-light text-sm leading-relaxed">{{ Str::limit($kb->description, 100) }}</p>
                    </div>
                    <a href="{{ route('panduan.showPublic', $kb->id) }}" class="relative z-10 inline-flex items-center text-sm font-semibold text-unmerBlue tracking-wide mt-auto group/btn">
                        Pelajari Dokumen
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-1 transform group-hover/btn:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                    </a>
                </div>
                @empty
