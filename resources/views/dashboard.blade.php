...
<body
    class="bg-[#f8fafc] text-gray-800 font-sans antialiased flex flex-col min-h-screen selection:bg-unmerBlue selection:text-white relative">
    ...
    <header class="glass-nav sticky top-0 z-50">
        <div class="container mx-auto px-6 py-4 flex items-center justify-between">
            <div class="flex items-center space-x-4 flex-1">
                <img src="{{ asset('images/logo(1).png') }}" alt="Logo PUSIM" class="h-11 w-auto rounded-md shadow-sm">
                <h1
            ...
            <nav
                class="hidden lg:flex items-center justify-center space-x-8 text-sm font-semibold tracking-wide text-gray-500 flex-1">
            ...
            <div class="flex flex-1 justify-end items-center gap-4">
                @auth
                    <div
                        class="flex items-center gap-3 bg-white px-4 py-2 rounded-full shadow-sm border border-gray-100/80">
                        <div
                            class="w-8 h-8 rounded-full bg-unmerBlue/10 text-unmerBlue flex items-center justify-center font-bold text-sm">
                            {{ substr(Auth::user()->name, 0, 1) }}
                        </div>
                        <span class="text-sm text-gray-700 font-medium hidden sm:block">{{ Auth::user()->name }}</span>
                        <form action="{{ route('logout') }}" method="POST"
                            class="inline ml-2 border-l border-gray-200 pl-3">
                            @csrf
                            <button type="submit"
                                class="font-medium text-gray-500 hover:text-red-500 text-sm transition-colors flex items-center gap-1.5 px-1 rounded-md">
    ...
    <main class="flex-grow container mx-auto px-6 py-10 max-w-7xl relative z-10 animate-fade-in">
        ...
        @if (session('success'))
            <div class="mb-6 bg-green-50 border border-green-200 text-green-700 px-5 py-4 rounded-xl relative flex items-center gap-3 shadow-sm animate-fade-in-up"
                role="alert">
        ...
        <!-- Welcome Banner -->
        <div
            class="bg-gradient-to-br from-[#1c5e82] via-[#164a66] to-[#0f3d59] rounded-[2rem] shadow-[0_20px_40px_-15px_rgba(28,94,130,0.4)] p-8 md:p-12 mb-10 text-white relative overflow-hidden animate-fade-in-up">
            <div class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10">
            </div>
            <div class="relative z-10 w-full md:w-2/3">
                <span
                    class="px-3 py-1 bg-white/10 border border-white/20 rounded-full text-xs font-semibold tracking-wider uppercase mb-5 inline-block backdrop-blur-sm">Akses
                    Utama</span>
                <h2 class="text-3xl md:text-4xl font-bold mb-4 leading-tight tracking-tight">Selamat Datang,
                    {{ Auth::user()->name ?? 'Pengguna' }} 👋</h2>
                <p class="text-blue-100/90 text-lg md:text-xl font-light mb-8 max-w-xl leading-relaxed">Kelola layanan
                    akademik dan dapatkan bantuan teknis PUSIM Unmer Malang dengan mudah.</p>
                <div class="flex flex-wrap gap-4">
                    <button onclick="openModal()"
                        class="bg-white text-unmerDark px-6 py-3 rounded-xl font-semibold shadow-lg hover:bg-gray-50 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl flex items-center gap-2">
                    ...
                    <a href="{{ url('/dashboard/basis-pengetahuan') }}"
                        class="bg-white/10 hover:bg-white/20 border border-white/30 backdrop-blur-md text-white px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="absolute right-0 top-0 h-full w-1/3 hidden lg:flex items-center justify-center opacity-20">
                <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg"
                    class="w-[300px] h-[300px] transform rotate-12">
                    <path fill="#ffffff"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10 animate-fade-in-up animate-delay-1">
            ...
            <div class="glass-panel rounded-2xl p-6 hover:-translate-y-1 transition-all duration-300 group">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span
                        class="text-[10px] font-bold px-2.5 py-1 bg-blue-100/50 text-blue-700 rounded-lg tracking-wide">+2
                        Hari Ini</span>
                </div>
            ...
            <div class="glass-panel rounded-2xl p-6 hover:-translate-y-1 transition-all duration-300 group">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span
                        class="text-[10px] font-bold px-2.5 py-1 bg-orange-100/50 text-orange-700 rounded-lg tracking-wide">Dalam
                        Proses</span>
                </div>
            ...
            <div class="glass-panel rounded-2xl p-6 hover:-translate-y-1 transition-all duration-300 group">
                <div class="flex items-center justify-between mb-4">
                    <div
                        class="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-500 flex items-center justify-center group-hover:scale-110 transition-transform">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span
                        class="text-[10px] font-bold px-2.5 py-1 bg-emerald-100/50 text-emerald-700 rounded-lg tracking-wide">Bulan
                        Ini</span>
                </div>
            ...
            <div class="glass-panel rounded-2xl p-6 hover:-translate-y-1 transition-all duration-300 group">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span
                        class="text-[10px] font-bold px-2.5 py-1 bg-red-100 text-red-600 rounded-lg tracking-wide">Penting</span>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10 animate-fade-in-up animate-delay-2">
            ...
            <div class="lg:col-span-2 glass-panel rounded-[1.5rem] overflow-hidden flex flex-col">
                <div class="px-7 py-5 border-b border-gray-100 flex justify-between items-center bg-white/50">
                    ...
                    <a href="#"
                        class="text-sm font-semibold text-unmerBlue hover:text-unmerDark transition-colors bg-blue-50 hover:bg-blue-100 px-4 py-2 rounded-lg">Lihat
                        Semua</a>
                </div>
                        ...
                        <tbody class="text-sm divide-y divide-gray-50">
                            @forelse($tickets as $ticket)
                                <tr class="hover:bg-blue-50/30 transition-colors group">
                                    <td class="px-7 py-4">
                                        <div
                                            class="font-semibold text-gray-900 group-hover:text-unmerBlue transition-colors">
                                            {{ $ticket->judul }}</div>
                                        <div class="text-gray-400 text-xs mt-0.5 font-mono">#{{ $ticket->ticket_number }}
                                        </div>
                                    </td>
                                    <td class="px-7 py-4 text-gray-600 text-sm">{{ $ticket->created_at->format('d M Y') }}
                                    </td>
                                    <td class="px-7 py-4">
                                        @if($ticket->status == 'menunggu')
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-[11px] font-bold bg-orange-50 text-orange-600 border border-orange-100">
                                                <span class="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse"></span>
                                                MENUNGGU
                                            </span>
                                        @elseif($ticket->status == 'diproses')
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-[11px] font-bold bg-blue-50 text-blue-600 border border-blue-100">
                                                <span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span> DIPROSES
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-[11px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100">
                                    ...
                                    <td class="px-7 py-4 text-right">
                                        <button type="button" onclick="showTicketDetail(this)"
                                            ...
                                            class="text-gray-400 hover:text-unmerBlue p-2 hover:bg-blue-50 rounded-lg transition-all group-hover:text-unmerBlue"
                                            title="Lihat Detail">
            ...
            <div class="glass-panel rounded-[1.5rem] overflow-hidden flex flex-col">
                ...
                <div class="p-5 space-y-2 flex-grow">
                    <a href="#" onclick="openModal(); return false;"
                        class="flex items-center p-3.5 rounded-xl hover:bg-blue-50/50 transition-all duration-300 border border-transparent hover:border-blue-100 group shadow-sm hover:shadow-md bg-white">
                        <div
                            class="w-10 h-10 bg-gradient-to-br from-unmerBlue to-unmerDark rounded-lg text-white flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform">
                        ...
                        <div class="ml-4">
                            <h4 class="font-bold text-gray-900 text-sm group-hover:text-unmerBlue transition-colors">
                                Buat Tiket Baru</h4>
                    ...
                    <a href="{{ route('profile.edit') }}"
                        class="flex items-center p-3.5 rounded-xl hover:bg-gray-50 transition-all duration-300 border border-gray-100 hover:border-gray-200 group shadow-sm hover:shadow bg-white">
                        <div
                            class="w-10 h-10 bg-gray-100 rounded-lg text-gray-500 flex items-center justify-center group-hover:bg-gray-200 group-hover:text-gray-700 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div
                    class="p-5 mt-auto bg-gradient-to-br from-emerald-50 flex items-center justify-between border-t border-emerald-100">
                    <div>
                    ...
                    <a href="https://wa.me/6281234567890" target="_blank"
                        class="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-medium text-xs rounded-lg transition-transform hover:scale-105 shadow-sm shadow-emerald-500/30">
                        Chat Sekarang
                    </a>
                </div>
            </div>
    ...
    <div id="ticketDetailModal" class="fixed inset-0 z-[999] hidden overflow-y-auto"
        ...
        <div class="fixed inset-0 bg-gray-900/60 transition-opacity backdrop-blur-sm" onclick="closeDetailModal()">
        </div>
            ...
            <div class="relative transform overflow-hidden rounded-[2rem] bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-100 opacity-0 translate-y-4 duration-300"
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-4 text-center sm:ml-5 sm:mt-1 sm:text-left w-full">
                            <h3 class="text-xl font-bold leading-6 text-gray-900 tracking-tight" id="detail-judul">
                                Detail Tiket</h3>
                            <p class="text-sm text-gray-500 mt-1 font-mono" id="detail-nomor"></p>

                            <div class="mt-6 space-y-4 text-sm text-gray-700">
                                <div>
                                    <span
                                        class="block font-semibold text-gray-500 text-xs uppercase tracking-wider mb-1">Status
                                        Layanan</span>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <span
                                            class="block font-semibold text-gray-500 text-xs uppercase tracking-wider mb-1">Kategori</span>
                                        <p id="detail-kategori"
                                            class="font-medium bg-gray-50 p-2.5 rounded-lg border border-gray-100 capitalize">
                                        </p>
                                    </div>
                                    <div>
                                        <span
                                            class="block font-semibold text-gray-500 text-xs uppercase tracking-wider mb-1">Tanggal
                                            Pengajuan</span>
                                        <p id="detail-tanggal"
                                            class="font-medium bg-gray-50 p-2.5 rounded-lg border border-gray-100"></p>
                                    </div>
                                </div>
                                <div>
                                    <span
                                        class="block font-semibold text-gray-500 text-xs uppercase tracking-wider mb-1">Deskripsi
                                        Kendala</span>
                                    <div id="detail-deskripsi"
                                        class="font-medium bg-gray-50 p-3.5 rounded-xl border border-gray-100 whitespace-pre-wrap text-gray-800 text-sm leading-relaxed min-h-[80px]">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-gray-50 px-6 py-5 sm:flex sm:flex-row-reverse rounded-b-[2rem]">
                    <button type="button" onclick="closeDetailModal()"
                        class="w-full inline-flex justify-center rounded-xl border border-transparent bg-unmerBlue px-6 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-unmerDark sm:ml-3 sm:w-auto transition-colors">
                        Tutup
                    </button>
                </div>
            </div>
            ...
            <div class="relative transform overflow-hidden rounded-[2rem] bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-100 opacity-0 translate-y-4 duration-300"
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-4 text-center sm:ml-5 sm:mt-1 sm:text-left w-full">
                            ...
                            <form id="ticketForm" class="mt-6 space-y-5" action="{{ route('tickets.store') }}"
                                ...
                                <div>
                                    <label for="judul"
                                        class="block text-sm font-semibold text-gray-700 text-left mb-1.5">Judul
                                        Topik</label>
                                    <input type="text" name="judul" id="judul" required
                                        class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-unmerBlue focus:ring-unmerBlue sm:text-sm p-3 bg-gray-50 focus:bg-white transition-colors"
                                        placeholder="Misal: Lupa Password E-learning">
                                </div>

                                <div>
                                    <label for="kategori"
                                        class="block text-sm font-semibold text-gray-700 text-left mb-1.5">Kategori
                                        Layanan</label>
                                ...
                                <div>
                                    <label for="deskripsi"
                                        class="block text-sm font-semibold text-gray-700 text-left mb-1.5">Rincian
                                        Deskripsi</label>
                                ...
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 text-left mb-1.5">Lampirkan
                                        Bukti (Opsional)</label>
                                    <div
                                        class="flex justify-center rounded-xl border-2 border-dashed border-gray-300 px-6 py-6 hover:bg-gray-50 hover:border-unmerBlue/50 transition-colors">
                                        <div class="space-y-1 text-center">
                                            ...
                                            <div class="flex text-sm text-gray-600 justify-center mt-2">
                                                <label for="file-upload"
                                                    class="relative cursor-pointer rounded-md bg-transparent font-semibold text-unmerBlue hover:text-unmerDark">
                ...
                <div class="bg-gray-50 px-6 py-5 sm:flex sm:flex-row-reverse rounded-b-[2rem]">
                    <button type="submit" form="ticketForm"
                        class="btn-elegant w-full inline-flex justify-center rounded-xl border border-transparent px-6 py-2.5 text-sm font-semibold text-white shadow-sm sm:ml-3 sm:w-auto">
                        Kirim Tiket
                    </button>
                    <button type="button" onclick="closeModal()"
                        class="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-200 bg-white px-6 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 sm:mt-0 sm:w-auto transition-colors">
                        Batal
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function showTicketDetail(button) {
            ...
            let statusHtml = '';
            if (status === 'menunggu') {
                statusHtml = `<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-[11px] font-bold bg-orange-50 text-orange-600 border border-orange-100">
                    <span class="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse"></span> MENUNGGU
                </span>`;
            } else if (status === 'diproses') {
                statusHtml = `<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-[11px] font-bold bg-blue-50 text-blue-600 border border-blue-100">
                    <span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span> DIPROSES
                </span>`;
            } else {
                statusHtml = `<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-[11px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> SELESAI
                </span>`;
            }
    ...
    <footer class="bg-white/80 backdrop-blur-md py-8 border-t border-gray-200 mt-auto relative z-10">
        <div
            ...
            <div class="flex items-center space-x-3 mb-4 md:mb-0">
                <div
                    class="w-8 h-8 rounded-full bg-unmerBlue flex items-center justify-center text-white font-bold text-xs shadow-sm">
                    P</div>
